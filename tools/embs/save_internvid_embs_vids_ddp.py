...
def main(args):
    ...
    print(f"Process {args.shard_id}: Creating model on {device}")
    ...
    Qformer_model = DDP(Qformer_model, device_ids=[local_rank])
    ...
    for video_ids, f_idxs, frames in tqdm(loader):
        ...
        frm_feats = model.module.encode_vision(frames, test=True)[0]
        ...
        # import pdb; pdb.set_trace()
        query_output = Qformer_model.module.bert(
...
if __name__ == "__main__":
    ...
    # Set shard ID dynamically
    args.shard_id = local_rank
    args.num_shards = torch.distributed.get_world_size()
