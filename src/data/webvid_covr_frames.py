        ...
        if isinstance(items[0], torch.Tensor):
            ...
            if all(item.shape == items[0].shape for item in items):
                ...
                if items[0].dim() == 1:  # Handling 1D tensors (e.g., sequences)
                    ...
                    # Determine max shape across all items
                    max_shape = torch.tensor([list(item.shape) for item in items]).max(dim=0)[0].tolist()
                    
                    padded_items = []
                    for item in items:
                        # Compute padding for each dimension
                        padding = []
                        for item_dim, max_dim in zip(item.shape[::-1], max_shape[::-1]):
                            pad_value = max_dim - item_dim
                            if pad_value < 0:
                                raise ValueError(f"Negative padding encountered: {pad_value}")
                            padding.extend([0, pad_value])  # (left_pad, right_pad) format

                        padded_items.append(torch.nn.functional.pad(item, padding))

                    batch_dict[key] = torch.stack(padded_items)
        else:
            batch_dict[key] = items  # Non-tensor items (e.g., strings)
...
class WebVidCoVRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if self.emb_pool == "mean":
        return_dict["tar_img_feat"] = sampled_target_emb
