...
def id2seq(data, seq_map, sub_format="{:05d}"):
    """
    Assigns sequential numbers for non-digit replacements in data.
    """
    if isinstance(data, list):
        return [replace_non_digits_with_seq(d, seq_map, sub_format) for d in data]
    else:
        return replace_non_digits_with_seq(data, seq_map, sub_format)


def replace_non_digits_with_seq(string, seq_map, sub_format="{:05d}"):
    """
    Replaces non-digits in a string with sequential numbers from seq_map.
    """
    if string not in seq_map:
        # Assign the next available sequence number
        seq_map[string] = int(sub_format.format(len(seq_map) + 1))
    return seq_map[string]
...
class FrameLoader:
    ...
    end_time: Union[float, None] = None,
        ...
        """
        Efficiently extracts video frames using decord.VideoReader.
    
        Args:
            video_pth (str): Path to the video file.
            start_time (float): Start time in seconds. Default is 0.0.
            end_time (float, optional): End time in seconds. If None, extracts till the end.
    
        Returns:
            list: List of transformed video frames.
        """
