...
class LanguageBindCir(Blip2Base):
    """
    ...
    def __init__(
        ...
        logging.info("using MLP with LanguageBind")
        # import pdb; pdb.set_trace()
    ...
    def forward(self, batch, fabric):
        ...
        # Text
        inputs = {
            # 'query_video': to_device(modality_transform['video'](query_list), device),
            # 'target_video': to_device(modality_transform['video'](target_list), device),
            'language': to_device(tokenizer(caption, max_length=77, padding='max_length',
        ...
        with torch.no_grad():
            outputs = self.model({
                # 'video': inputs['query_video'],  # Run query video
                'language': inputs['language']
            })
            # query_embeds = outputs['video']
            language_embeds = outputs['language']
...
def languagebind_cir(model, ckpt_path, **kwargs):
    if ckpt_path:
