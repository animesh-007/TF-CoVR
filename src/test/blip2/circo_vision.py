...
class TestCirco:
    ...
    def __call__(self, model, data_loader, fabric):
        ...
        if fabric.global_rank == 0:
            ...
            # print(f" len of ref_img_ids {len(ref_img_ids)}")
            # print(f" len of query_feats {len(query_feats)}")
            # print(f" len of query_ids {len(query_ids)}")
            ...
            # tar_img_ids = torch.Tensor(tar_img_ids).long()
            ...
            for i, (query_id, query_sims) in enumerate(zip(query_ids, sims_q2t)):
                sorted_indices = np.argsort(query_sims)[::-1]
